...
describe("keyboard controls", () => {
  it("maps WASD, arrows, Space, N, and Escape by physical key code", () => {
    ...
    expect(controlKeyForCode("Space")).toBe("BRAKE");
  ...
  it("neutralises opposing axes and gives Space priority", () => {
  ...
  it("enables Nitro only while forward is held without reverse or brake", () => {
    ...
    expect(controlIntentFromPressedKeys(new Set(["KeyW", "KeyN", "Space"])).nitro).toBe(false);
  });
});
